...
export class ConfusionMatrix {
   ...
   */
  constructor(
    confusionMatrix?: { labels: Array<string>; matrix: Array<Array<number>> },
  ...
  setConfusionMatrix(
    ...
    if (confusionMatrix) {
      ...
      this._matrix = this.deepCopy<Array<Array<number>>>(
        confusionMatrix._matrix
      );
      if (saveInHistory) {
        this.addToHistory();
      }
  ...
  labelAccuracy(label: string): number {
    ...
    const { truePositive, trueNegative, falsePositive, falseNegative } =
      this.getConfusionMatrixClasses(label);
    const result =
  ...
  matrixAccuracy(average = AverageMethod.Weighted): number {
    ...
    switch (average) {
      case AverageMethod.Micro:
        return this.microAccuracy();
      case AverageMethod.Macro:
        return this.macroAccuracy();
  ...
  microAccuracy(): number {
    const { truePositive, trueNegative, falsePositive, falseNegative } =
      this.getSumConfusionMatrixClasses();
    const result =
      (truePositive + trueNegative) /
      (truePositive + trueNegative + falsePositive + falseNegative);
    return result || 0;
  ...
  weightedAccuracy(): number {
    ...
    this._labels.forEach(
      (label, index) => (sum += this.labelAccuracy(label) * sumLabels[index])
    );
  ...
  matrixMissClassificationRate(average = AverageMethod.Weighted): number {
    ...
    switch (average) {
      case AverageMethod.Micro:
        return this.microMissClassificationRate();
      case AverageMethod.Macro:
        return this.macroMissClassificationRate();
      case AverageMethod.Weighted:
        return this.weightedMissClassificationRate();
    }
  }
  ...
  macroMissClassificationRate(): number {
    let sum = 0;
    this._labels.forEach(
      (label) => (sum += this.labelMissClassificationRate(label))
    );
    const result = sum / this._labels.length;
    return result || 0;
  }
  ...
  weightedMissClassificationRate(): number {
    ...
    this._labels.forEach(
      (label, index) =>
        (sum += this.labelMissClassificationRate(label) * sumLabels[index])
    );
    const result = sum / numberOfPredictions;
    return result || 0;
  }
  ...
  labelPrecision(label: string): number {
    this.validate();
    const { truePositive, falsePositive } =
      this.getConfusionMatrixClasses(label);
  ...
  weightedPrecision(): number {
    ...
    let sum = 0;
    this._labels.forEach(
      (label, index) => (sum += this.labelPrecision(label) * sumLabels[index])
    );
    const result = sum / numberOfPredictions;
    return result || 0;
  }
  ...
  labelRecall(label: string): number {
    this.validate();
    const { truePositive, falseNegative } =
      this.getConfusionMatrixClasses(label);
  ...
  weightedRecall(): number {
    ...
    let sum = 0;
    this._labels.forEach(
      (label, index) => (sum += this.labelRecall(label) * sumLabels[index])
    );
    const result = sum / numberOfPredictions;
    return result || 0;
  }
  ...
  matrixSpecificity(average = AverageMethod.Weighted): number {
    ...
    switch (average) {
      case AverageMethod.Micro:
        return this.microSpecificity();
      case AverageMethod.Macro:
        return this.macroSpecificity();
      case AverageMethod.Weighted:
        return this.weightedSpecificity();
    }
  }
  ...
  weightedSpecificity(): number {
    ...
    let sum = 0;
    this._labels.forEach(
      (label, index) => (sum += this.labelSpecificity(label) * sumLabels[index])
    );
    const result = sum / numberOfPredictions;
    return result || 0;
  }
  ...
  labelSpecificity(label: string): number {
    this.validate();
    const { trueNegative, falsePositive } =
      this.getConfusionMatrixClasses(label);
    const result = trueNegative / (trueNegative + falsePositive);
    return result || 0;
  }
   ...
   */
  f1Score(configuration?: { label?: string; average?: AverageMethod }): number {
    this.validate();
    if (configuration?.label && configuration?.label.length > 0) {
  ...
  weightedF1Score(): number {
    ...
    let sum = 0;
    this._labels.forEach(
      (label, index) => (sum += this.labelF1Score(label) * sumLabels[index])
    );
    const result = sum / numberOfPredictions;
    return result || 0;
  }
   ...
   */
  getAllMatrixClasses(): Array<{
    label: string;
    confusionMatrixClasses: ConfusionMatrixClasses;
  }> {
    this.validate();
    const all = new Array<{
      label: string;
  ...
  getConfusionMatrixClasses(label: string): ConfusionMatrixClasses {
    ...
    const falsePositive =
      this._matrix[position].reduce((previous, next) => previous + next) -
      truePositive;
    ...
    const trueNegative =
      numberOfPredictions - truePositive - falsePositive - falseNegative;
  ...
  clone(withoutHistory = false): ConfusionMatrix {
    ...
    cloneObj.normalizations = this.deepCopy<Array<ConfusionMatrix>>(
      this.normalizations
    );
    if (!withoutHistory) {
  ...
  getLabelsPredictionsSum(): Array<number> {
    let sumLabels = new Array<number>(this._labels.length).fill(
      0,
      0,
      this._labels.length
    );
  ...
  validate(confusionMatrix: ConfusionMatrix = this): ConfusionMatrix {
    if (confusionMatrix._labels.length !== confusionMatrix._matrix.length) {
      throw new Error(
        "The labels length should be equals to the matrix columns length."
      );
    }
    ...
    for (let i = 0; i < confusionMatrix._labels.length - 1; i++) {
      for (let j = i + 1; j < confusionMatrix._labels.length; j++) {
        if (
          confusionMatrix._labels[i].toLocaleLowerCase() ===
          confusionMatrix._labels[j].toLowerCase()
        ) {
          throw new Error(
            `The label ${confusionMatrix._labels[i]} appears more than once in the labels array.`
          );
        }
      }
    }
    ...
    confusionMatrix._matrix.forEach((array) => {
      if (array.length !== confusionMatrix._matrix.length) {
        throw new Error(
          "The confusion matrix does not have the columns/rows length."
        );
      }
    });
  ...
  transpose(): ConfusionMatrix {
    this.addToHistory();
    this._matrix = this._matrix[0].map((col, i) =>
      this._matrix.map((row) => row[i])
    );
    return this;
  }
  ...
  isRedoAvailable(): boolean {
    return (
      this.history.length > 1 && this.historyPointer < this.history.length - 1
    );
  }
  ...
  addLabel(
    ...
    if (rowValues[index] !== columnsValues[index]) {
      throw new Error(
        `The value form rowValues on position ${index} (value=${rowValues[index]}) should be the same in columnsValues position ${index} (value=${columnsValues[index]}).`
      );
    }

    if (rowValues.length !== columnsValues.length) {
      throw new Error(
        `The rows as columns arrays should have the same length.`
      );
    }

    if (rowValues.length !== this._labels.length + 1) {
      throw new Error(
        `The rows values length should be ${
          this._labels.length + 1
        } instead of ${rowValues.length}.`
      );
    }
  ...
  changeLabelOrder(
    ...
    if (this._matrix.length < 2) {
      throw new Error(
        `It is not possible to change label order on a confusion matrix with columns/rows < 2`
      );
    }

    if (firstLabelPosition < 0 || firstLabelPosition >= this._matrix.length) {
      throw new Error(
        `The firstLabelPosition value should be between [0, ${this._matrix.length}[`
      );
    }

    if (secondLabelPosition < 0 || secondLabelPosition >= this._matrix.length) {
      throw new Error(
        `The secondLabelPosition value should be between [0, ${this._matrix.length}[`
      );
    }
  ...
  importAsJson(json: string): ConfusionMatrix {
    try {
      ...
      throw new Error(
        `It was not possible to import the json.\n Details:\n${ex}`
      );
    }
    return this;
  }
